...
import com.example.programacionReactivaProyect.Domain.Exceptions.DuplicateSubscriptionException;
...
public class CreateSubscriptionUseCase {
    ...
    private Mono<Subscription> checkDuplicateSubscription(Subscription subscription) {
        return subscriptionGateway.getSubscriptionByIdUserAndCity(
                        subscription.getIdUser(),
                        subscription.getCity()
                )
                .flatMap(existing ->
                        Mono.<Subscription>error(new DuplicateSubscriptionException(
                                "Already subscribed to " + subscription.getCity()
                        ))
                )
                .switchIfEmpty(Mono.just(subscription));  // ← Si no existe, continuar
    }
